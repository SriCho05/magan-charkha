      ...
      <DialogContent onInteractOutside={(e) => e.preventDefault()} hideCloseButton>
        <DialogHeader>
          ...
          <DialogDescription className="text-center">
            Scan the QR code with your UPI app to pay <span className="font-bold">Rs. {cartTotal.toFixed(2)}</span>.
          </DialogDescription>
        </DialogHeader>
...
export default function CheckoutPage() {
  ...
  }, [user, authLoading, cartItems, router, toast, form, isOrderPlaced]);

  const handleSuccessfulOrder = async (shippingData: ShippingFormValues) => {
    if (!user || !user.email) return;

    setIsOrderPlaced(true); // Prevent redirect when cart is cleared
    ...
    try {
      ...
      throw error; // Let onSubmit handle it
    }
  };
  ...
  const getButtonText = () => {
    ...
    if (paymentMethod === 'card') return `Pay Rs. ${cartTotal.toFixed(2)}`;
